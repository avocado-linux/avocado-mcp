...
class DatabaseManager {
    ...
    CACHE_DIR = path.join(process.cwd(), ".avocado-cache");
    ...
    async downloadFile(url, outputPath) {
        ...
        return {
            contentLength: response.headers.get("content-length") || buffer.byteLength.toString(),
    ...
    async decompressFile(inputPath, outputPath) {
        ...
        if (!sqliteHeader.startsWith("SQLite format 3")) {
            throw new Error(`Decompressed file is not a valid SQLite database. Header: ${sqliteHeader.slice(0, 16).replace(/\0/g, "\\0")}`);
        }
        return {
    ...
    async introspectDatabase(dbPath) {
        ...
        if (!fileHeader.startsWith("SQLite format 3")) {
            throw new Error(`File is not a valid SQLite database: ${dbPath}. Header: ${fileHeader.replace(/\0/g, "\\0")}`);
        }
            ...
            for (const repo of repos) {
                try {
                    ...
                    if (metadata.primaryDb) {
                        ...
                        const compressedDbPath = path.join(this.CACHE_DIR, `${target}-${repo.replace(/\//g, "_")}-primary${path.extname(metadata.primaryDb)}`);
...
export function registerConsolidatedTools(server) {
    ...
    server.tool("get-config-schema", "REQUIRED: Acquire the JSON schema for Avocado OS configurations. This schema is essential for validating all Avocado configurations and must be obtained before generating any configuration files.", {
        version: z
            .string()
            .optional()
            .describe("Git tag/version to fetch (e.g., 'v1.0.0', 'main'). If not provided, fetches the latest version from main branch"),
    }, async ({ version }) => {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Schema not found for version '${schemaVersion}'. HTTP ${response.status}: ${response.statusText}`);
            }
    ...
    server.tool("query-repos", "Perfect for package searches during configuration generation. Execute SQL queries against Avocado OS repository databases with automatic preparation and includes schema information and formatted result tables.", {
        targets: z
            .array(z.string())
            .describe("List of target platforms to query databases for (e.g., ['jetson-orin-nano-devkit-nvme', 'raspberrypi4'])"),
        query: z
            .string()
            .describe("SQL query to execute against the package databases (e.g., \"SELECT name FROM packages WHERE name LIKE '%kernel%'\")"),
    }, async ({ targets, query }) => {
        try {
