export function registerSchemaTools(server) {
    server.tool("get-schema-info", "Get JSON schemas specifying the Avocado config structure", {}, async () => {
...
- Use GitHub's file browser to locate schema files`;
    ...
    server.tool("get-schema-download-info", "Get information on how to discover and download Avocado configuration schemas from GitHub - USE THIS FIRST", {}, async () => {
...
**Note:** Always discover schema files from the actual repository rather than assuming specific URLs.`;
    ...
    server.tool("get-schema-validation-patterns", "Get guidance on how to validate configurations against JSON schemas", {}, async () => {
