...
export function registerConfigTools(server) {
    server.tool("validate-config-structure", "Validate a TOML configuration against both TOML syntax and Avocado schema structure", {
        config_content: z
            .string()
        ...
        lines.forEach((line, index) => {
            ...
            if (trimmed.startsWith("[") && trimmed.endsWith("]")) {
                ...
                if (deprecatedSections.includes(section)) {
                    issues.push(`Line ${index + 1}: Section [${section}] is not valid in Avocado schema`);
                }
    ...
    server.tool("get-config-guidelines", "Get style and structure guidelines for Avocado configuration files", {}, async () => {
...
**Critical Reminder:** Configuration generation without schema-first workflow will result in invalid configurations. Always download schema first, then verify packages, then generate configuration.`;
    ...
    server.tool("get-extension-patterns", "Get information about Avocado extension configuration patterns", {}, async () => {
