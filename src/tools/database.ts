...
class DatabaseManager {
  ...
  private readonly CACHE_DIR = path.join(os.tmpdir(), "avocado-mcp-databases");
  private preparedDatabases: Map<string, DatabaseInfo[]> = new Map();
  ...
  private async decompressFile(
    ...
    if (sourcePath.endsWith(".bz2")) {
      ...
      throw new Error(
        "XZ decompression requires external tools. Please decompress manually or use a different format.",
      );
    } else {
  ...
  private async introspectDatabase(
    dbPath: string,
  ): Promise<{ tables: string[]; schema: { [tableName: string]: string[] } }> {
  ...
  async prepareTargetDatabases(targets: string[]): Promise<string> {
    let output = `# Database Preparation Results\n\n`;
    const allDatabaseInfo: DatabaseInfo[] = [];
    ...
    for (const target of targets) {
      ...
      for (const repo of repos) {
        ...
        try {
          ...
          // Parse repomd.xml
          const repomdContent = await import("fs").then((fs) =>
            fs.promises.readFile(repomdPath, "utf8"),
          );
          ...
          if (metadata.primaryDb) {
            ...
            const compressedDbPath = path.join(
              this.CACHE_DIR,
              `${target}-${repo.replace(/\//g, "_")}-primary.db.compressed`,
            );
            ...
            // Download database
            const dbDownloadInfo = await this.downloadFile(
              dbUrl,
              compressedDbPath,
            );
            ...
            if (
              ...
              try {
                const decompressionInfo = await this.decompressFile(
                  compressedDbPath,
                  dbPath,
                );
              ...
              await import("fs").then((fs) =>
                fs.promises.rename(compressedDbPath, dbPath),
              );
            }
  ...
  async queryDatabases(targets: string[], sqlQuery: string): Promise<string> {
  ...
  getPreparedTargets(): string[] {
    return Array.from(this.preparedDatabases.keys());
  }

  getDatabaseInfo(target: string): DatabaseInfo[] | undefined {
...
export function registerDatabaseTools(server: McpServer) {
  ...
  server.tool(
    ...
    "Download, decompress, and prepare SQLite databases for one or more targets",
    {
      targets: z
        .array(z.string())
  ...
  server.tool(
    "query-databases",
    "Execute SQL queries against prepared databases with schema information",
    {
      targets: z
        .array(z.string())
  ...
  server.tool(
    "list-prepared-databases",
    "List all currently prepared database targets and their schema information",
    {},
    async () => {
