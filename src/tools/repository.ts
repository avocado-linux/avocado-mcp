...
class RepositoryResource {
  ...
  async getSupportedTargets(): Promise<string[]> {
...
export function registerRepositoryTools(server: McpServer) {
  ...
  server.tool(
    "download-file",
    "Download one or multiple files from URLs (useful for repository metadata, schemas, etc.) - For SQLite databases, use prepare-target-databases instead for better efficiency",
    {
  ...
  server.tool(
    "get-repository-databases",
    "Get the SQLite database URLs for a target to enable package searches",
    {
    ...
    async ({ target }) => {
      ...
      const repos =
        await repositoryResource.getRepositoryPathsForTarget(target);
  ...
  server.tool(
    "list-targets",
    "List all supported target platforms from targets.json",
    {},
    async () => {
  ...
  server.tool(
    "get-target-repos",
    "Get repository paths for a specific target from targets.json mapping",
    {
      target: z.string().describe("Target platform name"),
    },
    async ({ target }) => {
  ...
  server.tool(
    "get-repository-info",
    "Get information about the Avocado package repository structure and targets.json mapping",
    {},
    async () => {
